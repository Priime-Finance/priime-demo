...
OPERATOR=$(role_addr owner)
SIGNING_KEY=$(cast wallet address --mnemonic "$MNEMONIC" --mnemonic-index 1)

# --- 0. preconditions -------------------------------------------------------
cast block-number --rpc-url "$RPC" >/dev/null 2>&1 \
  || { echo "FATAL: no chain reachable at $RPC (TARGET=$TARGET; for the fork: anvil --host 0.0.0.0 --block-time 1)"; exit 1; }
...
# Force the deploy's last transaction into a block before the next read. On
# the fork that is anvil_mine; on a live chain we wait for a real one.
mine_or_wait
...
"${CLI[@]}" workflow trigger  --id "$WID" set-cron --schedule "$CRON_SCHEDULE" --start-time "$START" --end-time "$END" >/dev/null
