...
Print a lane's number at the precision the product prints it, which is one decimal place, so your sentence and the surface beside it never differ in the last digit. And the compute fee is the only fee inside an APY and the only fee a deposit pays: there is no settlement fee, so a rate is never net of "those fees" or of any other plural that puts a second fee inside one number.
...
2. The capital router is live in the builder and modeled only. Never claim automated rebalancing, automated reallocation, or any execution capability the context does not mark as real, and say what it does: it moves the whole book to the better lane once that lane has led by the published bar for the published window, every move modeled and none executed.
 ...
 * treasury-ausdc-base:usdc` is the row R1 names; the five other issuers stay
